        ...
        for (int i = 0; i < amount; i++){
            ...
            switch (type){
                case 0:
                    ...
                    productSorter.sortProduct(new FoodProduct(name, productID));
                    break;

                case 1:
                    name = healthProducts[random.nextInt(healthProducts.length)];
                    productSorter.sortProduct(new HealthProduct(name, productID));
                    break;

                case 2:
                    name = electricProducts[random.nextInt(electricProducts.length)];
                    productSorter.sortProduct(new ElectricProduct(name, productID));
                    break;
            }
        }
    }
